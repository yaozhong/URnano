...
# 20190516 autocode update setting for the server
# 20190615 revision to the 11 coding system
...
def train_unet(args, modelSavePath="../experiment/model/", curveMonitor=True):

    lossType = args.loss

    print("\n"+"-"*50)
    print("Start nanopore base-calling model training")
    print("-"*50)
    ...
    """
    print("@ Prepare weights for the loss function (Training only) ... ")
    weightsY = Y2weights(label_seg)
    weightsY = np.repeat(np.expand_dims(weightsY, -1), 8, axis=-1)
    print(weightsY.shape)
    # add the weights to the label
    trainY = weightsY*trainY
    """
    ...
    if args.lstm == 0:
        ...
        print("** START module of UNet-GRU1")
        model = UNet_GRU1(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])
    elif args.lstm == 2:
        print("** START module of UNet-GRU2")
        model = UNet_GRU2(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])
        ...
        print("** START module of UNet-GRU4")
        model = UNet_GRU4(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])

    elif args.lstm == 5:
        print("** START new module of Multi-scale UNet-GRU3")
        model = MUNet_GRU3(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])
        ...
        print("** START UNet_GRU3_xceptionTest ...")
        model = UNet_GRU3_xceptionTest(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])

    elif args.lstm == 12:
        print("** START new module of MIX-UNET-RNN")
        ...
        print("** START new module of MIX-UNET-RNN_RNNUremove")
        model = UNet_LSTM_MIX_RNNUremove(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])
        #plot_model(model, to_file='../experiment/devLOG/figures/MIX_UNet_RNN_model.png')

    elif args.lstm == 14:
        print("** START new module of URnet-test fc")
        model = URnet_test(signals, params["kernel_size"], params["conv_window_len"],  params["maxpooling_len"],  \
            params["stride"], True, params["DropoutRate"])
    ...
    #elif lossType == "weight_categorical_loss": # weighted loss is for labels
    #    model.compile(optimizer=Adam(params["lr"]), loss = [ 'categorical_crossentropy' ] , metrics=[ metrics.categorical_accuracy ])
